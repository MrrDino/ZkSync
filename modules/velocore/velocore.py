...
from velocore import constants as cst
from velocore.abis.pair import PAIR_ABI
from velocore.abis.router import ROUTER_ABI
from velocore.abis.factory import FACTORY_ABI
...
class Velocore(SimpleW3):
    ...
    async def add_liquidity(self, token0: str, key: str, exchange: str, mode: int = 1) -> bool:
        ...
        try:
            ...
            logger.info(
                f'||ADD LIQ {token_in}/{token_out}| {liq_tx} '
                f'Gas: {gas} gwei, \33[{36}m{tx_fee}\033[0m'
            )
        except Exception as err:
            logger.error(f"\33[{31}m{err}\033[0m")
    ...
    async def get_usd_value(
        ...
        amount_data = await router.functions.getAmountOut(amount_in, token0, token1).call()
    ...
    async def get_fee(
        ...
        fee = await self.get_usd_value(amount_in=amount, router=router, token0=token0, token1=token1)
